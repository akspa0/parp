...
class ADTFile:
    ...
    def _parse_file(self):
        ...
        with open(self.filename, 'rb') as f:
            # First chunk should be MHDR
            mhdr_header = self._read_chunk_header(f)
            if not mhdr_header or mhdr_header.magic != 'MHDR':
                raise ValueError("Invalid ADT file: MHDR not found")
            
            # Parse MHDR flags
            self.mhdr_flags = MHDRFlags(struct.unpack('<I', f.read(4))[0])
            f.seek(mhdr_header.offset + mhdr_header.size)  # Skip rest of MHDR
            
            # Parse remaining root chunks
            while True:
                ...
                if chunk_header.magic == 'MCIN':
                    ...
                    mcnk = MCNKChunk(chunk_data, self.version)

...
def normalize_filename(fname):
    # Convert to lowercase, use forward slashes, and strip leading slash/dot
...
def main(directory, listfile_path, db_path="analysis.db"):
    ...
    if os.path.exists(listfile_path):
        with open(listfile_path, 'r', encoding='utf-8', errors='replace') as lf:
            for line in lf:
                if line.strip():
                    known_good_files.add(normalize_filename(line.strip()))
    ...
    all_unique_ids = []
    missing_files_reported = set()  # to prevent duplicate logging per run
    ...
    for filename in os.listdir(directory):
        if filename.lower().endswith(".adt"):
            ...
            for tex in parsed['textures']:
                ...
                if known_good_files and not check_file_in_listfile(tex, known_good_files):
                    key = (normalize_filename(tex), filename)
                    if key not in missing_files_reported:
                        missing_files_reported.add(key)
                        missing_logger.info(f"Missing file: {tex} referenced by {filename}")

            # Check M2 models
            for mm in parsed['m2_models']:
                c.execute("INSERT INTO m2_models (adt_id, model_name) VALUES (?,?)", (adt_id, mm))
                if known_good_files and mm and not check_file_in_listfile(mm, known_good_files):
                    key = (normalize_filename(mm), filename)
                    if key not in missing_files_reported:
                        missing_files_reported.add(key)
                        missing_logger.info(f"Missing file: {mm} referenced by {filename}")

            # Check WMO models
            for wm in parsed['wmo_models']:
                c.execute("INSERT INTO wmo_models (adt_id, wmo_name) VALUES (?,?)", (adt_id, wm))
                if known_good_files and wm and not check_file_in_listfile(wm, known_good_files):
                    key = (normalize_filename(wm), filename)
                    if key not in missing_files_reported:
                        missing_files_reported.add(key)
                        missing_logger.info(f"Missing file: {wm} referenced by {filename}")
            ...
            for m in parsed['mddf']:
                ...
                if known_good_files and m['model_name'] and not check_file_in_listfile(m['model_name'], known_good_files):
                    key = (normalize_filename(m['model_name']), filename)
                    if key not in missing_files_reported:
                        missing_files_reported.add(key)
                        missing_logger.info(f"Missing file: {m['model_name']} referenced by {filename}")
            ...
            for w in parsed['modf']:
                ...
                if known_good_files and w['wmo_name'] and not check_file_in_listfile(w['wmo_name'], known_good_files):
                    key = (normalize_filename(w['wmo_name']), filename)
                    if key not in missing_files_reported:
                        missing_files_reported.add(key)
                        missing_logger.info(f"Missing file: {w['wmo_name']} referenced by {filename}")

...
from terrain_structures import (
    ...
    ModelPlacement, WMOPlacement, MCNKInfo, MapTile
)
...
def insert_model_placement(conn: sqlite3.Connection, file_id: int, placement: Union[ModelPlacement, WMOPlacement],
                         model_id: int, tile_x: int, tile_y: int) -> int:
    """Insert model placement record"""
    c = conn.cursor()
    
    # Common placement data
    params = [
        file_id, tile_x, tile_y, model_id,
        placement.unique_id,
    ...
    # Add WMO-specific data if available
    if isinstance(placement, WMOPlacement):
        params.extend([
            placement.doodad_set,
            placement.name_set,
            placement.bounding_box.min.x,
            placement.bounding_box.min.y,
            placement.bounding_box.min.z,
            placement.bounding_box.max.x,
            placement.bounding_box.max.y,
            placement.bounding_box.max.z
        ])
    else:
        params.extend([None] * 8)  # Null for WMO-specific fields
    
    c.execute("""
        INSERT INTO model_placements
        (file_id, tile_x, tile_y, model_id, unique_id,

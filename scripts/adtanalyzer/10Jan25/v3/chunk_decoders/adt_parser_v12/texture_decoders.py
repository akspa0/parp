...
class TextureLayer:
    ...
    effect_id: int
    blend_mode: int
    alpha_map: Optional[bytes] = None  # Added field for alpha map data
...
class TextureDecoder:
    """Decoder for texture-related chunks (MTEX, MTXF, MCLY, MCAL)"""
    ...
    @staticmethod
    def decode_mcal(data: bytes, layers: List[TextureLayer]) -> Dict[int, bytes]:
        """Decode MCAL chunk containing alpha maps for texture layers"""
        try:
            alpha_maps = {}
            current_pos = 0
            
            for layer_idx, layer in enumerate(layers):
                if layer.has_alpha_map:
                    alpha_map = bytearray()
                    
                    if layer.is_compressed:
                        # Decompress RLE encoded alpha map
                        while len(alpha_map) < 4096 and current_pos < len(data):
                            command = data[current_pos]
                            fill_mode = bool(command & 0x80)
                            count = command & 0x7F
                            current_pos += 1
                            
                            if fill_mode and current_pos < len(data):
                                value = data[current_pos]
                                alpha_map.extend([value] * count)
                                current_pos += 1
                            elif not fill_mode and current_pos + count <= len(data):
                                alpha_map.extend(data[current_pos:current_pos + count])
                                current_pos += count
                    else:
                        # Raw alpha map data
                        size = 4096 if current_pos + 4096 <= len(data) else 2048
                        if size == 2048:
                            # 4-bit alpha values
                            raw_data = data[current_pos:current_pos + size]
                            for byte in raw_data:
                                alpha_map.append((byte & 0xF) * 16)
                                alpha_map.append((byte >> 4) * 16)
                        else:
                            # 8-bit alpha values
                            alpha_map.extend(data[current_pos:current_pos + size])
                        current_pos += size
                    
                    if alpha_map:
                        alpha_maps[layer_idx] = bytes(alpha_map[:4096])  # Ensure 64x64 size

            return alpha_maps
        except Exception as e:
            logger.error(f"Error decoding MCAL chunk: {e}")
            return {}
class TextureManager:
    ...
    def load_from_chunks(self, mtex_data: bytes, mtxf_data: Optional[bytes] = None, 
                        mcly_data: Optional[bytes] = None, mcal_data: Optional[bytes] = None) -> None:
        """Load texture definitions from MTEX and optional MTXF/MCLY/MCAL chunk data"""
        # Decode filenames
        ...
        if mcal_data and layers:
            alpha_maps = TextureDecoder.decode_mcal(mcal_data, layers)
            for layer_idx, alpha_map in alpha_maps.items():
                if layer_idx < len(layers):
                    layers[layer_idx].alpha_map = alpha_map
            ...
            for texture in sorted(self.textures.values(), key=lambda x: x.filename):
                ...
                if texture.layers:
                    ...
                    for layer in texture.layers:
                        ...
                        if layer.alpha_map:
                            f.write(f"  - Alpha Map Size: {len(layer.alpha_map)} bytes\n")
...
def example_usage():
    ...
    # Example alpha map data (simplified)
    mcal_data = bytes([
        0x81, 0xFF,  # RLE command: fill 1 byte with 0xFF
        0x02, 0x80, 0x40  # Raw copy of 2 bytes
    ])
    
    # Create texture manager
    manager = TextureManager()
    manager.load_from_chunks(mtex_data, mtxf_data, mcly_data, mcal_data)
    ...
    for texture in manager.get_all_textures():
        ...
        if texture.layers:
            ...
            for layer in texture.layers:
                ...
                if layer.alpha_map:
                    print(f"  Alpha Map Size: {len(layer.alpha_map)} bytes")

...
class ADTStructures:
    ...
    def decode_mcin(data: bytes) -> Dict[str, Any]:
        ...
        entries = []
        entry_size = 16  # 4 uint32s per entry
        expected_entries = 256
        
        if len(data) < entry_size:
            return {
                'error': f"MCIN data too small: {len(data)} bytes, minimum {entry_size} required",
                'raw_data': data.hex()
            }
    ...
    def decode_mmdx(data: bytes) -> Dict[str, List[str]]:
        ...
        return {'filenames': read_c_string_list(data)}

    @staticmethod
    def decode_mmid(data: bytes) -> Dict[str, List[int]]:
        """Decode M2 instance ID list"""
        count = len(data) // 4
        return {'ids': list(struct.unpack(f'{count}I', data))}
    ...
    def decode_mcrf(data: bytes) -> Dict[str, List[int]]:
        ...
        count = len(data) // 4
        return {'references': list(struct.unpack(f'{count}I', data))}

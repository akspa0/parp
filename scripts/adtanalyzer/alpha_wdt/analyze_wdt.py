...
from other_utils.decode_chunks import decode_MCNK, decode_MCVT, decode_MCNR, decode_MCLY, decode_MCAL, decode_MCSH, decode_MCLQ, decode_MCCV
    ...
    try:
        with WDTFile(Path(filepath)) as wdt:
            ...
            while pos < len(wdt.mm):
                ...
                # Check for Retail format indicators
                elif chunk_name in ['MMDX', 'MWMO']:
                    is_retail = True
                    print("Detected Retail WDT format")
            ...
            while pos < len(wdt.mm):
                ...
                # For Retail format, use specialized decoders for certain chunks
                if is_retail and chunk_size > 0:
                    chunk_data = wdt.mm[pos+8:pos+8+chunk_size]
                    if chunk_name == 'MCNK':
                        decoded_data, _ = decode_MCNK(chunk_data)
                        if 'sub_chunks' in decoded_data:
                            # Process sub-chunks like MCVT, MCNR, etc.
                            for sub_chunk_name, sub_chunk_data in decoded_data['sub_chunks'].items():
                                if sub_chunk_name == 'MCVT':
                                    heights = sub_chunk_data.get('heights', [])
                                    if heights:
                                        insert_height_map(conn, wdt_id, -1, -1, heights)
                                elif sub_chunk_name == 'MCLY':
                                    layers = sub_chunk_data.get('layers', [])
                                    for layer in layers:
                                        insert_tile_layer(conn, wdt_id, -1, -1,
                                                        layer['texture_id'],
                                                        layer['flags'],
                                                        layer.get('offset_in_mcal', 0))
                                elif sub_chunk_name == 'MCLQ':
                                    liquid_data = sub_chunk_data.get('heights', [])
                                    if liquid_data:
                                        insert_liquid_data(conn, wdt_id, -1, -1, liquid_data)

...
def parse_main(data, wdt_file=None):
    ...
    if len(data) != 64 * 64 * entry_size:
        logging.error(f"Invalid MAIN chunk size. Expected {64 * 64 * entry_size}, got {len(data)}")
        return {'entries': []}
    ...
    for y in range(64):
        for x in range(64):
            ...
            # Only process and log if the tile exists (has ADT data)

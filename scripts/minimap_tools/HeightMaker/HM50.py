...
def close_holes_and_export(vertices, faces, output_file, export_format):
    try:
        mesh = stl_mesh.Mesh(np.zeros(0), np.zeros(0))
        mesh.update_vertices(np.array(vertices))
        mesh.update_faces(np.array(faces))
        if export_format == 'obj':
            mesh.export(output_file + '.obj')
            print(f"3D model with closed holes exported as {output_file}.obj (OBJ format)")
        elif export_format == 'stl':
            mesh.save(output_file + '.stl', mode=stl_mesh.Mode.ASCII, update_normals=False, name='mesh')
            print(f"3D model with closed holes exported as {output_file}.stl (STL format)")
        else:
            print("Unsupported export format. Please choose either 'obj' or 'stl'.")
    except Exception as e:
        print(f"Error occurred while exporting 3D model: {e}")
...
def main():
    ...
    parser.add_argument("--close-holes", action="store_true", help="Close holes in the 3D model before exporting")
    ...
    if args.close_holes:
        for file in os.listdir(args.output_dir):
            if file.endswith("_merged"):
                ...
                close_holes_and_export(file_path + '.obj', args.export_format)

...
def crop_into_tiles(image, tile_size, output_folder):
...
def main(input_image_path, output_folder, tile_size):
    # Load the image
    original_image = Image.open(input_image_path)

    # Crop into tiles and get non-blank tiles
    non_blank_tiles = crop_into_tiles(original_image, tile_size, output_folder)

    # Build new image with non-blank tiles
    new_image = build_new_image(original_image, non_blank_tiles, tile_size)
    new_image_path = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(input_image_path))[0]}_new.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tile-size", type=int, default=64, help="Size of each tile")
    args = parser.parse_args()

    main(args.input_image, args.output_folder, args.tile_size)

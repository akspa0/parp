...
from wmo_root_chunk_decoders import wmo_root_chunk_decoders, parse_wmo_root
from wmo_group_chunk_decoders import wmo_group_chunk_decoders, parse_wmo_group
from common_helpers import ensure_folder_exists, convert_to_json, analyze_data_type, parse_and_split_fields, save_json, reverse_chunk_id
import struct
...
def decode_chunks(data, pm4_chunk_decoders):
    detailed_chunks = []
    ...
    for chunk in data:
        ...
        decoder = pm4_chunk_decoders.get(chunk_id) or pm4_chunk_decoders.get(reverse_chunk_id(chunk_id))
        if decoder:
            try:
...
def process_file(input_file, output_dir, output_json):
    ...
    try:
        ...
        elif file_name.lower().endswith(('pm4', 'pd4', 'wmo')):
            if file_name.lower().endswith('pd4'):
                parsed_data = parse_pd4(input_file)
                detailed_data = decode_chunks(chunks, pd4_chunk_decoders)
            elif file_name.lower().endswith('wmo'):
                if "_group" in file_name.lower():
                    parsed_data = parse_wmo_group(input_file)
                    detailed_data = decode_chunks(chunks, wmo_group_chunk_decoders)
                else:
                    parsed_data = parse_wmo_root(input_file)
                    detailed_data = decode_chunks(chunks, wmo_root_chunk_decoders)
            else:
                detailed_data = decode_chunks(chunks, pm4_chunk_decoders)
        ...
        if output_json:
            initial_output = os.path.join(output_json, file_name.replace('.pm4', '_initial_analysis.json').replace('.adt', '_initial_analysis.json').replace('.pd4', '_initial_analysis.json').replace('.wmo', '_initial_analysis.json'))
            detailed_output = os.path.join(output_json, file_name.replace('.pm4', '_detailed_analysis.json').replace('.adt', '_detailed_analysis.json').replace('.pd4', '_detailed_analysis.json').replace('.wmo', '_detailed_analysis.json'))
...
def main():
    parser = argparse.ArgumentParser(description="Process PM4, PD4, ADT, and WMO files, store data in SQLite databases, and optionally export to JSON.")
    ...
    if os.path.isdir(args.input_path):
        files = [os.path.join(root, file) for root, _, files in os.walk(args.input_path) for file in files if file.lower().endswith(('.pm4', '.pd4', '.adt', '.wmo'))]

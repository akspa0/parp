...
def parse_wlw_or_wlm_file(file_path, is_wlm=False, verbose=False):
    ...
    with open(file_path, 'rb') as file:
        ...
        result = {
            ...
            'liquid_type_str': LIQUID_TYPE_MAPPING.get(liquid_type, ('unknown', 'Yellow.png'))[0],
            'texture': LIQUID_TYPE_MAPPING.get(liquid_type, ('unknown', 'Yellow.png'))[1],
            'padding': padding,
...
def generate_combined_obj_file(file_path, analysis_result, output_dir, relative_path, verbose=False):
    """
    Generates a combined OBJ file representing the heightmap from all blocks in a WLW/WLM file.
    """
    if verbose:
        print(f"Generating combined OBJ file for: {file_path}")
    ...
    if verbose:
        print(f"Combined OBJ file and MTL file generated: {obj_filename} and {mtl_filename}")
...
def analyze_files(input_directory, output_dir, verbose=False):
    ...
    Generates combined OBJ files for visualizing the heightmaps.
    """
    if verbose:
        ...
        for file in files:
            ...
            if file.endswith('.wlw') or file.endswith('.wlm') or file.endswith('.wlw.not') or file.endswith('.wlm.not'):
                ...
                try:
                    ...
                    generate_combined_obj_file(file_path, analysis_result, output_dir, relative_root, verbose)

...
namespace ModernWoWTools.ADTMeta.Analysis.Services
{
    ...
    public class ReferenceValidator
    {
        ...
        /// <param name="attemptRepair">Whether to attempt to repair invalid references.</param>
        /// <returns>The number of invalid references found.</returns>
        public int ValidateReferences(AdtAnalysisResult result, HashSet<string> knownGoodFiles, bool attemptRepair = true)
        {
            if (result == null)
            ...
            foreach (var reference in result.TextureReferences)
            {
                ...
                if (!reference.IsValid)
                {
                    ...
                    if (attemptRepair)
                    {
                        reference.RepairedPath = PathUtility.RepairPath(reference.OriginalPath);
                        if (reference.RepairedPath != null && knownGoodFiles.Contains(reference.RepairedPath))
                        {
                            _logger.LogDebug($"Repaired texture reference: {reference.NormalizedPath} -> {reference.RepairedPath}");
                        }
                    }
                }
            }
            ...
            foreach (var reference in result.ModelReferences)
            {
                ...
                if (!reference.IsValid)
                {
                    ...
                    if (attemptRepair)
                    {
                        ...
                        if (reference.RepairedPath != null && knownGoodFiles.Contains(reference.RepairedPath))
                        {
                            _logger.LogDebug($"Repaired model reference: {reference.NormalizedPath} -> {reference.RepairedPath}");
                        }
                    }
                }
            }
            ...
            foreach (var reference in result.WmoReferences)
            {
                ...
                if (!reference.IsValid)
                {
                    ...
                    if (attemptRepair)
                    {
                        ...
                        if (reference.RepairedPath != null && knownGoodFiles.Contains(reference.RepairedPath))
                        {
                            _logger.LogDebug($"Repaired WMO reference: {reference.NormalizedPath} -> {reference.RepairedPath}");
                        }
                    }
                }
            }

...
namespace ModernWoWTools.ADTMeta.Analysis.Services
{
    ...
    public class AdtAnalyzer
    {
            ...
            bool recursive = false)
        {
            ...
            foreach (var adtFile in adtFiles)
            {
                try
                {
                    ...
                    foreach (var reference in result.AllReferences.Where(r => !r.IsValid))
                    {
                        ...
                        // Track repaired paths
                        if (reference.RepairedPath != null)
                        {
                            summary.RepairedPaths++;
                            summary.RepairedPathMap[reference.NormalizedPath] = reference.RepairedPath;
                        }
                    }
            ...
            _logger.LogInfo($"Found {summary.MissingReferences} missing references and {summary.DuplicateIds} duplicate unique IDs.");

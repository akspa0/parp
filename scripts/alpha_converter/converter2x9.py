...
class WdtAlpha:
    def __init__(self, wdt_alpha_name):
        ...
        offset_in_file += 4
        current_chunk_size = struct.unpack('<I', file_content[offset_in_file:offset_in_file + 4])[0]
        offset_in_file += 4 + current_chunk_size
        ...
        mdnm_offset = struct.unpack('<I', file_content[mphd_start_offset + 4: mphd_start_offset + 8])[0]
        self.mdnm = self.read_chunk(file_content, mdnm_offset)

        monm_offset = struct.unpack('<I', file_content[mphd_start_offset + 12: mphd_start_offset + 16])[0]
        self.monm = self.read_chunk(file_content, monm_offset)
    ...
    def read_chunk(self, file_content, offset):
        ...
        return Chunk(name=chunk_name, size=chunk_size, data=list(chunk_data))
...
class Wdt:
    ...
    def toFile(self, file_name):
        whole_wdt = []
        if self.mver:
            whole_wdt.extend(self.mver.getWholeChunk())
        if self.mphd:
            whole_wdt.extend(self.mphd.getWholeChunk())
        if self.main:
            whole_wdt.extend(self.main.getWholeChunk())
        if self.mdnm:
            whole_wdt.extend(self.mdnm.getWholeChunk())
        if self.monm:
            whole_wdt.extend(self.monm.getWholeChunk())
        if self.modf:
            whole_wdt.extend(self.modf.getWholeChunk())

        logging.info(f"Writing {len(whole_wdt)} bytes to {file_name}")

        with open(file_name, 'wb') as f:
            f.write(bytearray(whole_wdt))
...
class AdtLk:
    ...
    def to_file(self, file_name):
        whole_adt = []
        if self.mhdr:
            whole_adt.extend(self.mhdr.getWholeChunk())
        if self.mcin:
            whole_adt.extend(self.mcin.getWholeChunk())
        if self.mtex:
            whole_adt.extend(self.mtex.getWholeChunk())
        if self.mddf:
            whole_adt.extend(self.mddf.getWholeChunk())
        if self.modf:
            whole_adt.extend(self.modf.getWholeChunk())

        for mcnk in self.mcnks:
            whole_adt.extend(mcnk.getWholeChunk())

        logging.info(f"Writing {len(whole_adt)} bytes to {file_name}")

        with open(file_name, 'wb') as f:
            f.write(bytearray(whole_adt))

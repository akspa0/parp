...
def convert_wdt_to_adts(input_file, output_directory):
    ...
    adt_count = min(len(mcnk_chunks), 64 * 64)
...
parser.addHere is a refined and concise approach to ensure the conversion script aligns correctly with the ADT/v18 file format details and handles the grid-based naming convention.

### Key Points:
1. **File Structure**: ADT files are 64x64 blocks, each containing 16x16 chunks.
2. **Chunk Handling**: Ensure essential chunks are handled (e.g., `MVER`, `MHDR`, `MCIN`, `MTEX`, `MCNK`).
3. **Naming Convention**: Output files should follow the `_XX_YY` format.

...
class WdtAlpha:
    ...
    def getExistingAdtsNumbers(self):
        ...
        for i in range(64 * 64):
            value = struct.unpack('<I', self.main.data[i * 4:(i + 1) * 4])[0]
...
def convert_wdt_to_adts(input_file, output_directory):
    ...
    adt_count = min(len(adt_numbers), len(adt_offsets))

    if adt_count == 0:
        logging.error("No ADTs found. Exiting.")
        return
    ...
    for i in range(adt_count):
        ...
        output_file = os.path.join(output_directory, f"output_{adt_alpha.get_x_coord():02d}_{adt_alpha.get_y_coord():02d}.adt")
